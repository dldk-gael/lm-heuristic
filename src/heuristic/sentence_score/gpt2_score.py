...
class GPT2Score(Score):
    ...
    def compute_score(self, sentences):
        ...
        def tokenize_and_add_bos_eos_tokens(text):
            return self.tokenizer.tokenize(self.tokenizer.bos_token + text + self.tokenizer.eos_token)

        # Prepare the input ids
        tokens_ids = [tokenize_and_add_bos_eos_tokens(sentence) for sentence in sentences]
        input_ids = torch.tensor(pad_sequence(tokens_ids, batch_first=True))
        ...
        with torch.no_grad():
            ...
            if i < input_ids.shape[0]:
                pred_scores.append(self.model(input_ids[i:]))
        ...
        for i in range(len(sentences)):
            ...
            sentence_token_scores = log_scores[i, range(len(sentence_token_ids)), tokens_ids]
